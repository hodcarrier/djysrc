...
void pciinfo(u32 BusNum)
{
    ...
    int ShortPCIListing;
        ...
        debug_printf("cmdpci","Scanning PCI devices on bus %d\r\n", BusNum);
    ShortPCIListing = 0;
    if (ShortPCIListing)
    {
        debug_printf("cmdpci","BusDevFun  VendorId   DeviceId   Device Class       Sub-Class\r\n");
        debug_printf("cmdpci","_____________________________________________________________\r\n");
    }
    ...
    for (Device = 0; Device < PCI_MAX_PCI_DEVICES; Device++)
    {
        ...
        for (Function = 0; Function < PCI_MAX_PCI_FUNCTIONS; Function++)
        {
            ...
            if (ShortPCIListing)
            {
                debug_printf("cmdpci","%02x %02x %02x   ", BusNum, Device, Function);
                pci_header_show_brief(dev);
            }
            else
            {
                debug_printf("cmdpci","Found PCI device %02x %02x %02x:\r\n",
...
void pci_header_show_brief(pci_dev_t dev)
{
    ...
    debug_printf("cmdpci","0x%4x     0x%4x     %-23s 0x%2x\r\n",
               vendor, device,
...
void cmd_pcie_print(const char *msg,pci_dev_t dev,int type,int reg)
{
    ...
    if(type == 2)
    {
        ...
        debug_printf("cmd_pci","msg = %s,pcie_byte = %d\r\n",msg, pcie_byte);
    }
    else if(type == 4)
    {
        pci_read_config_word(dev, reg, &pcie_word);
        debug_printf("cmd_pci","msg = %s,pcie_word = %d\r\n",msg, pcie_word);
    }
    else if(type == 8)
    {
        pci_read_config_dword(dev, reg, &pcie_dword);
        debug_printf("cmd_pci","msg = %s,pcie_dword = %d\r\n",msg, pcie_dword);
    }
}
/*
...
void pci_header_show(pci_dev_t dev)
{
    ...
    debug_printf("cmdpci","  class code =                  0x%2x (%s)\r\n", _byte,pci_class_str(_byte));
    ...
    default:
        debug_printf("cmdpci","unknown header\r\n");
        break;
    }
}
...
bool_t pcie(char *param)
{
    ...
    {
        debug_printf("cmdpci","输入的pcie设备 bus号超过了当前总线上的最大bus号\r\n");
        return false;
    }
    else
        pciinfo(busno);
